...
module.exports = {
    getAllFeeds: async (req, res) => {
        const feed = await Feed
            .find()
            .populate('addedBy')
            .populate('comments.commentBy').sort({ addedAt: -1 });
        res.send(feed);
    },
    likeFeed: async (req, res) => {
        ...
        Feed.findOneAndUpdate({ _id: req.params.id },
            ...
            async function (err, response) {
                const feed = await Feed.findById(req.params.id).populate('addedBy')
    ...
    addFeed: async (req, res) => {
        ...
        try {
            ...
            await feed.save();
            const feedd = await Feed.findById(feed._id).populate('addedBy')
                .populate('comments.commentBy');
            res.send(feedd)
